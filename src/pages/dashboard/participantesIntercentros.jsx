...
export const ParticipantesIntercentros = () => {
  ...
  // Check if enough teams are registered for the sorteo
  useEffect(() => {
  ...
  const sorteoIntercentros = async () => {
    // Esperar hasta que 'equiposInscritos' tenga los datos completos
    if (equiposInscritos.length === 3) {
      // console.log('equiposInscritos antes del sorteo:', equiposInscritos);

      // // Asegúrate de que cada equipo tenga un objeto `Equipo` válido
      // const equiposParaSorteo = equiposInscritos.map((inscrito) => inscrito.Equipo || null);
      
      // // Verifica si alguno de los equipos es null
      // if (equiposParaSorteo.includes(null)) {
      //   console.error("Uno o más equipos son inválidos:", equiposParaSorteo);
      //   notify("Error: Uno o más equipos son inválidos.");
      //   return;
      // }
      ...
      try {
        console.log('Data para sorteo:', data);  // Muestra la data antes de enviar
